...
这个项目全程使用 AIGC 编程工具协作。下面五次是有代表性的过程，都来自真实开发过程。

| 子任务 | AI 实现或提供了什么 | 效果如何 | 人工修改 |
|---|---|---|---|
| 结构与骨架 | `Arrow` / `GameState` 数据结构、`pygame` 主循环、棋子绘制骨架 | 能跑，但规则、生成器、界面全塞在一个文件里 | 我定下“关卡必须保证可解”这条硬性要求，让可解性检查成为生成流程的一部分 |
| 路径检测 | 整条射线扫描的 `arrow_can_leave_from_occupied` | 远距离阻挡、自己的尾巴阻挡都能识别 | 我补了贴边阻挡、贴边朝外飞出两组边界用例 |
| 关卡生成 | 带种子的 `build_level()` + 可解性校验 | 五关全部可解，箭头数逐关递增 | 我要求难度递增，并把五关都实际清了一遍 |
| 界面排错 | 按字宽和控件矩形重算排版 | 顶栏不再互相压住 | 我要求只挪文字锚点、不动点击热区，逐张截图核对 |
| 测试与修复 | 69 项 `unittest`，用 SDL dummy 无窗口驱动真实界面 | 测出一个真实缺陷：超时判负后棋盘卡死 | 我在 `shoot()` 守卫里补上 `self.game.dead` |
...
时间主要超在三处：界面细节反复调（顶栏排版返工两次、飞出动画节奏、日夜配色）；关卡
生成器为了保证“每关都可解”反复试参数；写测试时又测出超时卡死的缺陷，多花了一轮排查。
...
- [x] 多次有意义的 Commit（功能、测试、修复、样式、文档分开提交）；
